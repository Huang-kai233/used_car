...
五、筛选特征
"""
numerical_cols = df.select_dtypes(exclude='object').columns   #列的索引
...
output_path = 'C:/Users/黄凯/Documents/新建文件夹/信息文件/编程/实战项目/二手车/base test/'
tree_data = df
...
print(nn_data.shape)
train_num = df.shape[0]-50000
nn_data[0:int(train_num)].to_csv('C:/Users/黄凯/Documents/新建文件夹/信息文件/编程/实战项目/二手车/base test/'+'train_nn.csv', index=0, sep=' ')
nn_data[train_num:train_num+50000].to_csv('C:/Users/黄凯/Documents/新建文件夹/信息文件/编程/实战项目/二手车/base test/'+'test_nn.csv', index=0, sep=' ')
...
# print(train_tree.info())
#缩减特征向量，减少计算量，通过对比重要程度
# b = [Train_data]
# for k in b:
#     per = []
#     for i in k:
#         per.append([i, k['price'].corr(k[i])])
#     per3 = []
#     for i in per:
#         if -0.25<i[1]<0.25:
#             per3.append(i[0])
#             print(i[0],i[1])
#     per3.remove('SaleID')
#     k.drop(per3, axis=1, inplace=True)
#     TestA_data.drop(per3, axis=1, inplace=True)
# print('转换后')
...
output_path = os.path.abspath(os.path.dirname(os.getcwd()) + os.path.sep + ".")
# 测试集输出
predictions = predictions_lgb
predictions[predictions < 0] = 0
sub = pd.DataFrame()
sub['SaleID'] = TestA_data.SaleID
# sub['SaleID'] = TestA_data.SaleID[0:5000]
...
# sub['SaleID'] = Train_data.SaleID[0:5000]
sub['price'] = oof_lgb
...
# ## 读取神经网络模型数据
tree_data_path = os.path.abspath(os.path.dirname(os.getcwd()) + os.path.sep + ".")
...
for j in a:
    for i in j.columns:
        ...
        if dtype == 'float64':
            ...
            j[i] = j[i].astype(np.int32)
# print(Train_NN_data['price'])
# print(Test_NN_data['price'])
...
for train_index, vali_index in kfold:
    ...
    model.add(tensorflow_core.python.keras.layers.Dense(256, activation='relu', kernel_regularizer=tensorflow_core.python.keras.regularizers.l2(0.02)))
    model.add(tensorflow_core.python.keras.layers.Dense(128, activation='relu', kernel_regularizer=tensorflow_core.python.keras.regularizers.l2(0.02)))
    model.add(tensorflow_core.python.keras.layers.Dense(64, activation='relu', kernel_regularizer=tensorflow_core.python.keras.regularizers.l2(0.02)))
    model.add(tensorflow_core.python.keras.layers.Dense(1, kernel_regularizer=tensorflow_core.python.keras.regularizers.l2(0.02)))

    model.compile(
                loss='mean_absolute_error',
                #   # optimizer=tensorflow_core.python.keras.optimizers.Adam(),
...
# sub['SaleID'] = Test_NN_data.SaleID[0:5000]
sub['price'] = predictions
...
# sub['SaleID'] = Train_NN_data.SaleID[0:5000]
sub['price'] = oof_nn
...
#最后提交的结果值为454，排名170左右
#由于计算过程是在本人笔记本上运行，所以为了减少运算量，缩减了很多特征值，而且K折交叉运算折数也很少，导致目前的结果偏大。